...
def knapsack (items, rem_time, rlen) :
  print(rem_time, rlen)
  if rlen == 0 or rem_time <= 0:
    return []
  
  # if the time of current time is > rem_time, it cannot 
  # be included, exclude it
  if items[rlen - 1].time > rem_time :
    return knapsack(items, rem_time, rlen - 1)

  # Include the rlen - 1 item
  sack1 = knapsack(items, rem_time - items[rlen - 1].time, rlen - 1 )

  # Do not Include the rlen - 1 item
  sack2 = knapsack(items, rem_time, rlen - 1 )
  ...
  #print("Sack2")
  #print_sack(sack2)
  n1 = items[rlen - 1].payout + functools.reduce(lambda x,y: x + y.payout, sack1, 0)
  n2 = functools.reduce(lambda x,y: x + y.payout, sack2, 0)

  print("n1=%d, n2=%d" % (n1,n2))
  
  if n1 >= n2 :
    sack1.append(items[rlen - 1])
    return sack1
  else :
    return sack2
  
  
games = knapsack(items, 120, len(items))
